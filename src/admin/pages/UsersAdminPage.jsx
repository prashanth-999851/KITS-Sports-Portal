...
import { ShieldCheck, UserPlus, Ban, CheckCircle, X } from 'lucide-react';
...
export default function UsersAdminPage() {
  ...
  const handleSubmit = (e) => {
    ...
    addUser(formData);
    setShowModal(false);
    setFormData({ name: '', email: '', role: 'Sports Coordinator' });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border-color)] pb-4">
        <div>
          <h2 className="text-xl font-bold text-[var(--text-primary)]">User & RBAC Role Management</h2>
          <p className="text-xs text-[var(--text-muted)]">Manage admin accounts and assign Role-Based Access Control (RBAC) permissions.</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
          <div className="relative w-full max-w-lg p-6 rounded-xl glass-modal space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-3">
              <h3 className="text-base font-bold text-[var(--text-primary)]">Create Staff User</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-[var(--text-secondary)] mb-1 font-semibold">User Name *</label>
